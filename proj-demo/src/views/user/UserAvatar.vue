...
<template>
  <el-card class="page-container">
    ...
    <el-row>
      <el-col :span="12">
        <el-upload ref="upload" :auto-upload="true" :headers="{'Authorization': token}" :on-success="uploaded"
                   :show-file-list="false"
                   action="/api/upload" class="avatar-uploader" name="image">
          <img v-if="profile.avatar != undefined && profile.avatar.trim().length > 0" :src="`/${profile.avatar}`"
               alt="avatar"
               class="avatar" />
          <img v-else :src="png" alt="default" width="280" />
        </el-upload>
        <br />
        <el-button :icon="Plus" round size="large" @click="upload.$el.querySelector('input').click()">
          Select Image
        </el-button>
        <el-button :icon="Upload" round size="large" type="primary" @click="updateUserAvatar(profile.avatar as string)">
          Update Avatar
        </el-button>
      </el-col>
    </el-row>
